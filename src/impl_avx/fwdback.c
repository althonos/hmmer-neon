...
static inline void _mm256_store_ss(float* mem_addr, __m256 a) {
    _mm_store_ss(mem_addr, _mm256_castps256_ps128(a));
}
...
backward_engine(int do_full, const ESL_DSQ *dsq, int L, const P7_OPROFILE *om, const P7_OMX *fwd, P7_OMX *bck, float *opt_sc)
{
  ...
  dpv = esl_avx_leftshiftz_float(DMO(dpc,Q-1));            /* leftshift: [1 5 9 13 16 21 34 37] -> [5 9 13 16 21 34 37 x] */
  for (q = Q-1; q >= 0; q--)
    {
  ...
  for (i = L-1; i >= 1; i--)	/* backwards stride */
    {
      ...
      tmmv = esl_avx_leftshiftz_float(om->tfv[1]);
      timv = esl_avx_leftshiftz_float(om->tfv[2]);
      tdmv = esl_avx_leftshiftz_float(om->tfv[3]);
