...
p7_OptimalAccuracy(const P7_OPROFILE *om, const P7_OMX *pp, P7_OMX *ox, float *ret_e)
{
  ...
  register __m256 dcv;
  float  *xmx = ox->xmx;
  __m256 *dpc = ox->dpf[0];        /* current row, for use in {MDI}MO(dpp,q) access macro       */
  ...
  for (i = 1; i <= pp->L; i++)
    {
      ...
      mpv = esl_avx_rightshift_ps(MMO(dpp,Q-1), infv);  /* Right shifts by 4 bytes. 4,8,12,x becomes x,4,8,12. */
      dpv = esl_avx_rightshift_ps(DMO(dpp,Q-1), infv);
      ipv = esl_avx_rightshift_ps(IMO(dpp,Q-1), infv);
      for (q = 0; q < Q; q++)
	{
	  sv  =                   _mm256_and_ps(_mm256_cmp_ps(*tp, zerov, _CMP_GT_OS), xBv);  tp++;
	  sv  = _mm256_max_ps(sv, _mm256_and_ps(_mm256_cmp_ps(*tp, zerov, _CMP_GT_OS), mpv)); tp++;
	  sv  = _mm256_max_ps(sv, _mm256_and_ps(_mm256_cmp_ps(*tp, zerov, _CMP_GT_OS), ipv)); tp++;
	  sv  = _mm256_max_ps(sv, _mm256_and_ps(_mm256_cmp_ps(*tp, zerov, _CMP_GT_OS), dpv)); tp++;
	  ...
	  dcv = _mm256_and_ps(_mm256_cmp_ps(*tp, zerov, _CMP_GT_OS), sv); tp++;

	  sv         =                _mm256_and_ps(_mm256_cmp_ps(*tp, zerov, _CMP_GT_OS), mpv);   tp++;
	  sv         = _mm256_max_ps(sv, _mm256_and_ps(_mm256_cmp_ps(*tp, zerov, _CMP_GT_OS), ipv));  tp++;
       ...
       */
      dcv = esl_avx_rightshift_ps(dcv, infv);
      tp  = om->tfv + 7*Q;	/* set tp to start of the DD's */
      for (q = 0; q < Q; q++)
	{
	  DMO(dpc, q) = _mm256_max_ps(dcv, DMO(dpc, q));
	  dcv         = _mm256_and_ps(_mm256_cmp_ps(*tp, zerov, _CMP_GT_OS), DMO(dpc,q));   tp++;
	}
      ...
      for (j = 1; j < 4; j++)
	{
	  ...
	  for (q = 0; q < Q; q++)
	    {
	      DMO(dpc, q) = _mm256_max_ps(dcv, DMO(dpc, q));
	      dcv         = _mm256_and_ps(_mm256_cmp_ps(*tp, zerov, _CMP_GT_OS), dcv);   tp++;
	    }
	}
...
select_m(const P7_OPROFILE *om, const P7_OMX *ox, int i, int k)
{
  ...
  int     q     = (k-1) % Q;		/* (q,r) is position of the current DP cell M(i,k) */
  int     r     = (k-1) / Q;
  __m256 *tp    = om->tfv + 7*q;       	/* *tp now at start of transitions to cur cell M(i,k) */
  ...
  if (q > 0) {
    ...
    mpv = esl_avx_rightshiftz_float(ox->dpf[i-1][(Q-1)*3 + p7X_M]);
    dpv = esl_avx_rightshiftz_float(ox->dpf[i-1][(Q-1)*3 + p7X_D]);
    ipv = esl_avx_rightshiftz_float(ox->dpf[i-1][(Q-1)*3 + p7X_I]);
  }
...
select_d(const P7_OPROFILE *om, const P7_OMX *ox, int i, int k)
{
  ...
  if (q > 0) {
    ...
    mpv.v  = esl_avx_rightshiftz_float(ox->dpf[i][(Q-1)*3 + p7X_M]);
    dpv.v  = esl_avx_rightshiftz_float(ox->dpf[i][(Q-1)*3 + p7X_D]);
    tmdv.v = esl_avx_rightshiftz_float(om->tfv[7*(Q-1) + p7O_MD]);
    tddv.v = esl_avx_rightshiftz_float(om->tfv[8*Q-1]);
  }
